...
function configure_mmonit(){
    ...
    if [ $CONF_STATE -eq "0" ];then
        ...
        run "ansible-vault decrypt $MMONIT_LICENSE --vault-password-file $VAULT_PASSWORD"
        echo "${GREEN}MMonit License Decrypt Done${NC}"
        run "ansible-playbook $PLAYBOOK_HOME/monitoring//mmonit.yml"
        echo "${GREEN}MMonit Installation & Configuration Done!${NC}"
        run "ansible-vault encrypt $MMONIT_LICENSE --vault-password-file $VAULT_PASSWORD"
